...
import { getPublicFurnitureImgUrl } from '../../utils/getImgUrl'; // Use the correct function
...
const ProductCard = ({ product }) => {
    ...
    // Handle product card selection to set selected product name and navigate to capture page
    const handleProductSelect = () => {
        setSelectedProductName(product.name); // Set the selected product name
        navigate('/capture', { state: { product } }); // Navigate to the capture page with product details
    };

    return (
        <div key={product.id} className="border rounded-lg shadow-lg p-3 bg-white  w-full ">
            <div className="bg-[#FAFAFA] relative  flex justify-center items-center">
                {/* Use the correct function here */}
                <img
                    ref={imgRef}
                    src={getPublicFurnitureImgUrl(product.imageUrl)} // Updated
                    alt={product.name}
                    className="w-30 % h-30% object-cover px-8 py-2 rounded"
